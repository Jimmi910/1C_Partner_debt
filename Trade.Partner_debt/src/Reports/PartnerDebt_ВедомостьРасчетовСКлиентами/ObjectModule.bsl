...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	//МассивИменВыбранныхПолей.Удалить(МассивИменВыбранныхПолей.Найти("Лимит"));
	Индекс = МассивИменВыбранныхПолей.Найти("Лимит");
	Если НЕ Индекс = Неопределено Тогда
		МассивИменВыбранныхПолей.Удалить(Индекс);
	КонецЕсли;
	...
	// Вставляем Лимит Долга
	ТаблицаЛимитов = ЛимитыДолгаКонтрагентов();
	ВнейниеНаборыДанных = Новый Структура;
	ВнейниеНаборыДанных.Вставить("ЛимитыДолга", ТаблицаЛимитов);
	//
	
	ПроцессорКомпоновки = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКомпоновки.Инициализировать(МакетКомпоновки, ВнейниеНаборыДанных, ДанныеРасшифровки, Истина);
...
Функция ЛимитыДолгаКонтрагентов()
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	PartnerDebt_РучныеЛимитыДолгаПартнераСрезПоследних.Партнер КАК Партнер,
		|	PartnerDebt_РучныеЛимитыДолгаПартнераСрезПоследних.Сумма КАК Лимит,
		|	&Ручной КАК ТипЛимита
		|ИЗ
		|	РегистрСведений.PartnerDebt_РучныеЛимитыДолгаПартнера.СрезПоследних КАК
		|		PartnerDebt_РучныеЛимитыДолгаПартнераСрезПоследних
		|ГДЕ
		|	PartnerDebt_РучныеЛимитыДолгаПартнераСрезПоследних.Партнер.PartnerDebt_ТипУстановкиЛимитаДолга = &Ручной
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	Партнеры.Ссылка,
		|	0,
		|	Партнеры.PartnerDebt_ТипУстановкиЛимитаДолга
		|ИЗ
		|	Справочник.Партнеры КАК Партнеры
		|//ГДЕ
		|//	Партнеры.PartnerDebt_ТипУстановкиЛимитаДолга = &МаксимальныйИзДоговора";
	
	ТипМаксимальный = Перечисления.PartnerDebt_ТипУстановкиЛимитаДолга.МаксимальныйИзДоговора;
	
	Запрос.УстановитьПараметр("МаксимальныйИзДоговора", ТипМаксимальный);
	Запрос.УстановитьПараметр("Ручной", Перечисления.PartnerDebt_ТипУстановкиЛимитаДолга.Ручной);
	
	ТаблицаЛимитов = Запрос.Выполнить().Выгрузить();
	
	Для Каждого ТекущаяСтрока Из ТаблицаЛимитов Цикл
		Если ТекущаяСтрока.ТипЛимита = ТипМаксимальный Тогда
			ТекущаяСтрока.Лимит = PartnerDebt_КонтрольДолгаПартнераВызовСервера.ЛимитДолгаПартнера(ТекущаяСтрока.Партнер);
		КонецЕсли;
	КонецЦикла;
